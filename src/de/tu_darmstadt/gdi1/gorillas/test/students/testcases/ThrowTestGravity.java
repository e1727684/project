...
public class ThrowTestGravity {
	...
	public void testZeroGravity() {
		...
		Vector2f nextPosition1 = adapter.getNextShotPosition(new Vector2f(0, 400), 2, 2, 0, 0,
				true, 200);
	...
	public void testHigherGravityInfluence() {
		...
		Vector2f nextPosition1_1 = adapter.getNextShotPosition(new Vector2f(0, 400), 45, 80, 0, 10,
				true, 200);
		Vector2f nextPosition1_2 = adapter.getNextShotPosition(new Vector2f(0, 400), 45, 80, 0, 20,
				true, 200);

		assertTrue(
				"If the gravity is doubled a throw with apart from that same paramatrization should not fly as far as before anymore.",
				nextPosition1_2.y - nextPosition1_1.y > 0);
	}
	...
	private void testSingleThrow(float x0, float y0, int angle, int speed, int wind, int gravity,
		...
		Vector2f nextPosition = adapter.getNextShotPosition(new Vector2f(x0,
				y0), angle, speed, scaleWind(wind), gravity, fromLeftToRight, scaleTime(time));
		assertEquals("A shot, which is thrown from (" + x0 + ", " + y0
				+ ") with an angle of " + angle + " degree, a speed of "
				+ speed + ", wind of " + wind + " and a grvity of " + gravity + ", has to be at (" + expectedX + ", " + expectedY
