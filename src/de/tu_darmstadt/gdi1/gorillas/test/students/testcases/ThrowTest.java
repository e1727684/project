...
public class ThrowTest {
	...
	public void testLeftToRight() {
		...
		Vector2f nextPosition1 = adapter.getNextShotPosition(new Vector2f(0,
				400), 60, 80, true, 200);
		assertTrue(
				"A shot, which is thrown from left to right, has to move in positive x direction",
				nextPosition1.x > 0);
		Vector2f nextPosition2 = adapter.getNextShotPosition(new Vector2f(0,
				400), 60, 80, true, 400);
		assertTrue(
				"A shot, which is thrown from left to right, has to be further right after 200 ms passed. (If there is no wind.)",
				nextPosition2.x > nextPosition1.x);
		Vector2f nextPosition3 = adapter.getNextShotPosition(new Vector2f(0,
				400), 60, 80, true, 30000);
		assertTrue(
	...
	public void testRightToLeft() {
		...
		// angle of 60 degree and a speed of 80
		Vector2f nextPosition1 = adapter.getNextShotPosition(new Vector2f(600,
				400), 60, 80, false, 200);
		assertTrue(
				"A shot, which is thrown from right to left, has to move in negative x direction",
				nextPosition1.x < 600);
		Vector2f nextPosition2 = adapter.getNextShotPosition(new Vector2f(600,
				400), 60, 80, false, 400);
		assertTrue(
				"A shot, which is thrown from right to left, has to be further left after 200 ms passed. (If there is no wind.)",
				nextPosition2.x < nextPosition1.x);
		Vector2f nextPosition3 = adapter.getNextShotPosition(new Vector2f(0,
				400), 60, 80, true, 30000);
		assertTrue(
