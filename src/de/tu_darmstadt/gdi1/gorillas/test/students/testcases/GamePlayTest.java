...
public class GamePlayTest {
	...
	public void testGorillaHit() {
		...
		adapter.createCustomMap(1000, 600, 100, buildingCoordinates,
	...
	private void testLeftHitRight() {
		...
		assertTrue(
				"Player one hit player 2. 2000 ms afterwards it should be player two's turn.",
				adapter.isPlayer2Turn());
	}
	...
	public void testCityHit() {
		...
		assertEquals(
				"The right gorilla was not hit because the shot was blocked by the city, so the score of player one should be 0.",
				0, adapter.getPlayer1Score());
	}
	...
	public void testLeaveScreen() {
		...
		assertTrue(
				"The shot left the screen on the right. 2000 ms afterwards it should definitely be player two's turn.",
				adapter.isPlayer2Turn());
	}
}
