...
public class HighscoreTest {
	...
	public final void testCreateHighscoreEntry() {
		...
		assertEquals("Highscore count should be one after adding an entry", 1,
				adapter.getHighscoreCount());
		assertEquals("The playername of the highscore entry is incorrect", "PlayerOne",
				adapter.getNameAtHighscorePosition(0));
		assertEquals("The number of rounds played in total of the highscore entry is incorrect", 4,
				adapter.getRoundsPlayedAtHighscorePosition(0));
		assertEquals("The number of rounds won of the highscore entry is incorrect", 3,
				adapter.getRoundsWonAtHighscorePosition(0));
	}
	...
	public final void testNullAccess() {
		...
		assertNull("Accessing a non existent position should return null",
				adapter.getNameAtHighscorePosition(-1));
		assertNull("Accessing a non existent position should return null",
				adapter.getNameAtHighscorePosition(1));

		assertEquals("Accessing a non existent position should return null",-1,
				adapter.getMeanAccuracyAtHighscorePosition(-1), 0);
		assertEquals("Accessing a non existent position should return null", -1,
				adapter.getRoundsPlayedAtHighscorePosition(1));
