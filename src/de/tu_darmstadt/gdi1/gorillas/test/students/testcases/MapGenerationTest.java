...
public class MapGenerationTest {
	...
	public void testBuildingsValidity() {
		...
		ArrayList<Vector2f> buildingCoordinates = adapter
		...
		for (int i = 1; i < buildingCoordinates.size(); i++) {
			...
			assertTrue(
					"The buidlings should be ordered from left to right. The right neighbour"
							+ " of a building has to have a bigger x value than the building.",
					lastBuildingX < currentBuildingX);

			if (i == 1) {
				assertTrue(
						"The first building should not be higher than frameHight - 100.",
						lastBuildingY < adapter.getMapFrameHeight());
			}
			assertTrue(
					"The buildings should not be higher than frameHight - 100.",
					currentBuildingY < adapter.getMapFrameHeight());
		}

		assertTrue("There should be at least six buildings.",
				buildingCoordinates.size() >= 6);
	}
	...
	public void testGorillaValidity() {
		...
		for (int i = 1; i < buildingCoordinates.size(); i++) {
			...
			if (leftGorillaCoordinate.x <= currentBuildingX
					&& leftGorillaCoordinate.x >= lastBuildingX) {
				leftGorillaBuilding = i - 1;
			}

			if (rightGorillaCoordinate.x <= currentBuildingX
					&& rightGorillaCoordinate.x >= lastBuildingX) {
		...
		float leftGorillaBuildingWidth = buildingCoordinates
				.get(leftGorillaBuilding + 1).x
				- buildingCoordinates.get(leftGorillaBuilding).x;
		float expectedLeftGorillaX = buildingCoordinates
				.get(leftGorillaBuilding).x + leftGorillaBuildingWidth / 2;

		float rightGorillaBuildingWidth;
		if (!(rightGorillaBuilding == buildingCoordinates.size() - 1)) {
			rightGorillaBuildingWidth = buildingCoordinates
					.get(rightGorillaBuilding + 1).x
					- buildingCoordinates.get(rightGorillaBuilding).x;
		} else {
			rightGorillaBuildingWidth = adapter.getMapFrameWidth()
					- buildingCoordinates.get(rightGorillaBuilding).x;
		}

		float expectedRightGorillaX = buildingCoordinates
				.get(rightGorillaBuilding).x + rightGorillaBuildingWidth / 2;

		assertTrue(
				"The left gorilla should stand exactly in the middle of its building.",
